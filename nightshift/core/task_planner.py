...
"""
        ...
        try:
            ...
            # Build command without prompt (will use stdin instead)
            # Need to properly escape the JSON schema for shell
            escaped_schema = json_schema.replace('"', '\\"')
            cmd_str = f'{self.claude_bin} -p --output-format json --json-schema "{escaped_schema}"'

            # Use stdin to pass the prompt (avoids Windows command line length limits)
